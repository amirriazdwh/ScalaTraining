/************************************************************************************************
* A function works for every argument of the defined type. In other words, a function
* defined as (Int) => String takes any Int and returns a String.
* A Partial Function is only defined for certain values of the defined type. A Partial Function
* (Int) => String might not accept every Int.
* isDefinedAt is a method on PartialFunction that can be used to determine if the PartialFunction
* will accept a given argument.
...
* we have to send a function.  this is demonstrated below
  * you cannot use partical methods.  like def OneCase.  only partical Functions exists
**************************************************************************************************/
...
//
// use {} when using multiple line with ; or using partial functions
extensions.filter { case PhoneExt(name, extension) => extension < 200}
extensions.filter { case PhoneExt(name, extension) => extension < 200
                    case _ => false}
//
// a case statement is actually a Partical function. multiple case statements are being checked
//  with particalFunction2 orelse particalFunction2 orElse ParticalFunction2
...
// calling isDefinedAt from the case guard statement
...
val phoneExt = new PartialFunction[PhoneExt, List[String]] {
  def apply(i: PhoneExt):List[String]= List(i.name)
...
val greaterThan29:Any=>Int = {
  case i: Int if i > 20 => i
}

/*
 this has been valid and what actually happens
val greaterThan30:Any=>Int = PartialFunction[Any, Int]{
  case i: Int if i > 20 => i
}
*/

List(1, 45, 10, "blah", true, 25) collect greaterThan21


val division0: PartialFunction[(Double, Double), Double] = {
  case (x, y) if y != 0 => x /y
}
